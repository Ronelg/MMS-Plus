...
/**
 * This AsyncDialog class is used to execute a runnable in a background thread
 * and once that finishes, execute a runnable on the UI thread. If the
 * background runnable task takes longer than half a second, a progress modal
 * dialog is displayed.
 * 
 */
public class AsyncDialog {
	 ...
	 * @param postExecuteTask
	 *            an optional runnable to run on the UI thread when the
	 *            background runnable is finished
	 ...
	 * 
	 * TODO: Need to implement a way for the background thread to pass a result
	 * to the onPostExecute thread. AsyncTask already provides this
	 * functionality.
	 */
		...
		protected Void doInBackground(Runnable... params) {
			if (params != null) {
				try {
					...
					// Test code. Uncomment this block to test the progress
					// dialog popping up.
					// try {
					// Thread.sleep(2000);
					// } catch (Exception e) {
					...
					// Cancel pending display of the progress bar if the
					// background task has
