						...
						public void run() {
							...
							if (mThreadIds == null) {
								...
								Conversation.startDelete(mHandler, token,
										mDeleteLockedMessages, mThreadIds);
							}
						}
					});
			dialog.dismiss();
		...
		protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
			...
			case THREAD_LIST_QUERY_TOKEN:
				...
				if (mDoOnceAfterFirstQuery) {
					...
					// Delay doing a couple of DB operations until we've
					// initially queried the DB
					// for the list of conversations to display. We don't want
					// to slow down showing
					// the initial UI.

					// 1. Delete any obsolete threads. Obsolete threads are
					// threads that aren't
