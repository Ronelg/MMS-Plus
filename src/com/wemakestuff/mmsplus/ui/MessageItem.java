...
public class MessageItem {
	...
	MessageItem(Context context, String type, final Cursor cursor,
		...
		if ("sms".equals(type)) {
			...
			// Unless the message is currently in the progress of being sent, it
			// gets a time stamp.
			...
			if (!TextUtils.isEmpty(subject)) {
				...
				mSubject = MessageUtils.cleanseMmsSubject(context,
						v.getString());
			}
			...
			mAttachmentType = cursor.getInt(columnsMap.mColumnMmsTextOnly) != 0 ? WorkingMessage.TEXT
					: ATTACHMENT_TYPE_NOT_LOADED;

			// Start an async load of the pdu. If the pdu is already loaded, the
			// callback
			// will get called immediately
			boolean loadSlideshow = mMessageType != PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND;

			mItemLoadedFuture = MmsApp
					.getApplication()
					.getPduLoaderManager()
	...
	public boolean isMe() {
		// Logic matches MessageListAdapter.getItemViewType which is used to
		// decide which
		// type of MessageListItem to create: a left or right justified item
		// depending on whether
		// the message is incoming or outgoing.
		boolean isIncomingMms = isMms()
				&& (mBoxId == Mms.MESSAGE_BOX_INBOX || mBoxId == Mms.MESSAGE_BOX_ALL);
		boolean isIncomingSms = isSms()
				&& (mBoxId == Sms.MESSAGE_TYPE_INBOX || mBoxId == Sms.MESSAGE_TYPE_ALL);
	...
	public class PduLoadedMessageItemCallback implements ItemLoadedCallback {
		public void onItemLoaded(Object result, Throwable exception) {
			if (exception != null) {
				Log.e(TAG,
						"PduLoadedMessageItemCallback PDU couldn't be loaded: ",
						exception);
				return;
			}
			...
			if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
				...
				if (mMessageType == PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF) {
					...
					mContact = mAddress = mContext
							.getString(R.string.messagelist_sender_self);
					timestamp = msg == null ? 0
							: ((SendReq) msg).getDate() * 1000L;
				}

				SlideModel slide = mSlideshow == null ? null : mSlideshow
						.get(0);
				...
				mMessageSize = mSlideshow == null ? 0 : mSlideshow
						.getTotalMessageSize();

				String report = mCursor
						.getString(mColumnsMap.mColumnMmsDeliveryReport);
				if ((report == null)
						|| !mAddress.equals(mContext
								.getString(R.string.messagelist_sender_self))) {
					mDeliveryStatus = DeliveryStatus.NONE;
				} else {
					int reportInt;
			...
			if (!isOutgoingMessage()) {
				if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
					mTimestamp = mContext.getString(R.string.expire_on,
							MessageUtils.formatTimeStampString(mContext,
									timestamp));
				} else {
	...
	public interface PduLoadedCallback {
		 ...
		 * 
		 * @param messageItem
		 *            the MessageItem that finished loading.
		 */
