...
public class PhoneNumberUtils {
	...
	public static String getNumberFromIntent(Intent intent, Context context) {
		...
		final Cursor c = context.getContentResolver().query(uri,
				new String[] { phoneColumn }, null, null, null);
		if (c != null) {
			try {
				if (c.moveToFirst()) {
	 ...
	 * 
	 * This function is equivalent to extractNetworkPortion(), except for
	 * allowing the PLUS character to occur at arbitrary positions in the
	 * address portion, not just the first position.
	 * 
	 * @hide
	 */
	 ...
	 * become 1-800-4664-411), and then strips all separators (e.g.
	 * 1-800-4664-411 will become 18004664411).
	 * 
	...
	public static boolean compare(Context context, String a, String b) {
		return compare(a, b, MmsConfig.getStrictComparePhoneNumber());
	}
	...
	/**
	 * Returns the last numDigits of the reversed phone number Returns null if
	 * np == null
	 */
	...
	/**
	 * Returns the TOA for the given dial string Basically, returns
	 * TOA_International if there's a + prefix
	 */
	 ...
	 *            the data buffer
	 * @param offset
	 *            should point to the TOA (aka. TON/NPI) octet after the length
	 *            byte
	 * @param length
	 *            is the number of bytes including TOA byte and must be at least
	 *            2
	 * 
	 * @return partial string on invalid decode
	 * 
	 *         FIXME(mkf) support alphanumeric address type currently
	 *         implemented in SMSMessage.getAddress()
	 */
	public static String calledPartyBCDToString(byte[] bytes, int offset,
			int length) {
		...
		internalCalledPartyBCDFragmentToString(ret, bytes, offset + 1,
				length - 1);
	 ...
	 * For example: SIM ADN extension fields
	 */
	...
	/**
	 * Note: calls extractNetworkPortion(), so do not use for SIM EF[ADN] style
	 * records
	 * 
	...
	/**
	 * Same as {@link #networkPortionToCalledPartyBCD}, but includes a one-byte
	 * length prefix.
	 */
	 ...
	 * 
	 * @param number
	 *            dialing number string if the dialing number starts with '+',
	 *            set to international TOA
	...
	/**
	 * If includeLength is true, prepend a one-byte length value to the return
	 * array.
	 */
	private static byte[] numberToCalledPartyBCDHelper(String number,
		...
		int resultLen = (numberLenEffective + 1) / 2; // Encoded numbers require
														// only 4 bits each.
		int extraBytes = 1; // Prepended TOA byte.
		if (includeLength)
			extraBytes++; // Optional prepended length byte.
	...
	private static final String[] NANP_COUNTRIES = new String[] { "US", // United
																		// States
			"CA", // Canada
			"AS", // American Samoa
			"AI", // Anguilla
	...
	/**
	 * Breaks the given number down and formats it according to the rules for
	 ...
	 * {@link #FORMAT_NANP} and {@link #FORMAT_JAPAN} are supported as a
	 * formating type.
	 * 
	 * @param source
	 *            the phone number to format
	 * @param defaultFormattingType
	 *            The default formatting rules to apply if the number does not
	 *            begin with +[country_code]
	 ...
	 * @return The formatting type for the given locale, or FORMAT_UNKNOWN if
	 *         the formatting rules are not known for the given locale
	 */
	...
	/**
	 * Formats a phone number in-place. Currently {@link #FORMAT_JAPAN} and
	 * {@link #FORMAT_NANP} is supported as a second argument.
	 * 
	 * @param text
	 *            The number to be formatted, will be modified with the
	 *            formatting
	 * @param defaultFormattingType
	 *            The default formatting rules to apply if the number does not
	 *            begin with +[country_code]
	...
	/**
	 * Formats a phone number in-place using the NANP formatting rules. Numbers
	...
	public static void formatNanpNumber(Editable text) {
		...
		for (int i = 0; i < length; i++) {
			...
			case '0':
				...
				} else if (state != NANP_STATE_DASH
						&& (numDigits == 3 || numDigits == 6)) {
	...
	public static String formatNumber(String phoneNumber,
		...
		String result = null;
		try {
			PhoneNumber pn = util.parseAndKeepRawInput(phoneNumber,
					defaultCountryIso);
	 ...
	 * @param defaultCountryIso
	 *            the ISO 3166-1 two letters country code whose convention will
	 *            be used if the phoneNumberE164 is null or invalid, or if
	 *            phoneNumber contains IDD.
	 ...
	 */
	public static String formatNumber(String phoneNumber,
			String phoneNumberE164, String defaultCountryIso) {
		int len = phoneNumber.length();
		for (int i = 0; i < len; i++) {
		...
		if (phoneNumberE164 != null && phoneNumberE164.length() >= 2
			...
			try {
				// The number to be parsed is in E164 format, so the default
				// region used doesn't
				// matter.
				...
				// This makes sure phoneNumber doesn't contain an IDD
						normalizeNumber(phoneNumber).indexOf(
								phoneNumberE164.substring(1)) <= 0) {
	...
	public static String normalizeNumber(String phoneNumber) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (digit != -1) {
				...
				return normalizeNumber(PhoneNumberUtils
	...
	/**
	 * Checks a given number against the list of emergency numbers provided by
	 * the RIL and SIM card.
	 * 
	 * @param number
	 *            the number to look up.
	 * @return true if the number is in the list of emergency numbers listed in
	 *         the RIL / SIM, otherwise return false.
	 */
	...
	/**
	 * Checks if given number might *potentially* result in a call to an
	 * emergency service on the current network.
	 * 
	 * Specifically, this method will return true if the specified number is an
	 * emergency number according to the list managed by the RIL or SIM, *or* if
	 * the specified number simply starts with the same digits as any of the
	 * emergency numbers listed in the RIL / SIM.
	 * 
	 * This method is intended for internal use by the phone app when deciding
	 * whether to allow ACTION_CALL intents from 3rd party apps (where we're
	 * required to *not* allow emergency calls to be placed.)
	 * 
	 * @param number
	 *            the number to look up.
	 * @return true if the number is in the list of emergency numbers listed in
	 *         the RIL / SIM, *or* if the number starts with the same digits as
	 ...
	 * 
	 * @param useExactMatch
	 *            if true, consider a number to be an emergency number only if
	 *            it *exactly* matches a number listed in the RIL / SIM. If
	 *            false, a number is considered to be an emergency number if it
	 *            simply starts with the same digits as any of the emergency
	 *            numbers listed in the RIL / SIM. (Setting useExactMatch to
	 *            false allows you to identify number that could *potentially*
	 *            result in emergency calls since many networks will actually
	 *            ignore trailing digits after a valid emergency number.)
	 * 
	 * @return true if the number is in the list of emergency numbers listed in
	 *         the RIL / sim, otherwise return false.
	 */
	 ...
	 *            the number to look up.
	 * @param defaultCountryIso
	 *            the specific country which the number should be checked
	 *            against
	 * @return if the number is an emergency number for the specific country,
	...
	public static boolean isEmergencyNumber(String number,
			String defaultCountryIso) {
		return isEmergencyNumberInternal(number, defaultCountryIso, true /* useExactMatch */);
	}
	 ...
	 * 
	 * Specifically, this method will return true if the specified number is an
	 * emergency number in the specified country, *or* if the number simply
	 * starts with the same digits as any emergency number for that country.
	 * 
	 * This method is intended for internal use by the phone app when deciding
	 * whether to allow ACTION_CALL intents from 3rd party apps (where we're
	 * required to *not* allow emergency calls to be placed.)
	 ...
	 *         country, *or* if the number starts with the same digits as any of
	 *         those emergency numbers.
	 * 
	 * @hide
	 */
	public static boolean isPotentialEmergencyNumber(String number,
			String defaultCountryIso) {
		return isEmergencyNumberInternal(number, defaultCountryIso, false /* useExactMatch */);
	}
	 ...
	 *            false, a number is considered to be an emergency number if it
	 *            simply starts with the same digits as any of the emergency
	 *            numbers listed in the RIL / SIM.
	 * 
	 * @return true if the number is an emergency number for the specified
	 *         country.
	 */
	private static boolean isEmergencyNumberInternal(String number,
		...
		// (Be sure to do this check *before* calling
		...
		if (!TextUtils.isEmpty(numbers)) {
			...
			for (String emergencyNum : numbers.split(",")) {
				// It is not possible to append additional digits to an
		...
		if (defaultCountryIso != null) {
			...
			if (useExactMatch) {
				...
				return util
						.connectsToEmergencyNumber(number, defaultCountryIso);
			}
		} else {
			if (useExactMatch) {
	...
	/**
	 * Checks if a given number is an emergency number for the country that the
	 * user is in. The current country is determined using the CountryDetector.
	 * 
	 * @param number
	 *            the number to look up.
	 * @param context
	 *            the specific context which the number should be checked
	 *            against
	 * @return true if the specified number is an emergency number for a local
	 *         country, based on the CountryDetector.
	 * 
	 * @see android.location.CountryDetector
	 * @hide
	 */
	public static boolean isLocalEmergencyNumber(String number, Context context) {
		return isLocalEmergencyNumberInternal(number, context, true /* useExactMatch */);
	}
	 ...
	 * 
	 * Specifically, this method will return true if the specified number is an
	 * emergency number in the current country, *or* if the number simply starts
	 * with the same digits as any emergency number for the current country.
	 * 
	 * This method is intended for internal use by the phone app when deciding
	 * whether to allow ACTION_CALL intents from 3rd party apps (where we're
	 * required to *not* allow emergency calls to be placed.)
	...
	public static boolean isPotentialLocalEmergencyNumber(String number,
			Context context) {
		return isLocalEmergencyNumberInternal(number, context, false /* useExactMatch */);
	}
	...
	private static boolean isLocalEmergencyNumberInternal(String number,
			Context context, boolean useExactMatch) {
		String countryIso;
	...
	/**
	 * This function checks if there is a plus sign (+) in the passed-in dialing
	 * number. If there is, it processes the plus sign based on the default
	 * telephone numbering plan of the system when the phone is activated and
	 * the current telephone numbering plan of the system that the phone is
	 * camped on. Currently, we only support the case that the default and
	 * current telephone numbering plans are North American Numbering
	 * Plan(NANP).
	 * 
	 * The passed-in dialStr should only contain the valid format as described
	 * below, 1) the 1st character in the dialStr should be one of the really
	 * dialable characters listed below ISO-LATIN characters 0-9, *, # , + 2)
	 * the dialStr should already strip out the separator characters, every
	 * character in the dialStr should be one of the non separator characters
	 * listed below ISO-LATIN characters 0-9, *, # , +, WILD, WAIT, PAUSE
	 * 
	 ...
	 *         to NANP, and if there is the "+" in the original dial string.
	 *         Otherwise, the original dial string returns.
	 * 
	...
	public static String cdmaCheckAndProcessPlusCode(String dialStr) {
		if (!TextUtils.isEmpty(dialStr)) {
			if (isReallyDialable(dialStr.charAt(0)) && isNonSeparator(dialStr)) {
				String currIso = SystemProperties.get(
						PROPERTY_OPERATOR_ISO_COUNTRY, "");
				String defaultIso = SystemProperties.get(
						PROPERTY_ICC_OPERATOR_ISO_COUNTRY, "");
				if (!TextUtils.isEmpty(currIso)
						&& !TextUtils.isEmpty(defaultIso)) {
	...
	/**
	 * This function should be called from checkAndProcessPlusCode only And it
	 * is used for test purpose also.
	 * 
	 * It checks the dial string by looping through the network portion, post
	 * dial portion 1, post dial porting 2, etc. If there is any plus sign, then
	 * process the plus sign. Currently, this function supports the plus sign
	 * conversion within NANP only. Specifically, it handles the plus sign in
	 * the following ways: 1)+1NANP,remove +, e.g. +18475797000 is converted to
	 * 18475797000, 2)+NANP or +non-NANP Numbers,replace + with the current NANP
	 * IDP, e.g, +8475797000 is converted to 0118475797000, +11875767800 is
	 * converted to 01111875767800 3)+1NANP in post dial string(s), e.g.
	 ...
	 *            the original dial string
	 * @param currFormat
	 *            the numbering system of the current country that the phone is
	 *            camped on
	 * @param defaultFormat
	 *            the numbering system of the country that the phone is
	 *            activated on
	 * @return the converted dial string if the current/default countries belong
	 *         to NANP, and if there is the "+" in the original dial string.
	 *         Otherwise, the original dial string returns.
	...
	public static String cdmaCheckAndProcessPlusCodeByNumberFormat(
		...
		if (dialStr != null && dialStr.lastIndexOf(PLUS_SIGN_STRING) != -1) {
			// Format the string based on the rules for the country the number
			// is from,
			// and the current country the phone is camped on.
			if ((currFormat == defaultFormat) && (currFormat == FORMAT_NANP)) {
				// Handle case where default and current telephone numbering
				// plans are NANP.
				String postDialStr = null;
				String tempDialStr = dialStr;

				// Sets the retStr to null since the conversion will be
				// performed below.
				retStr = null;
				if (DBG)
					log("checkAndProcessPlusCode,dialStr=" + dialStr);
				// This routine is to process the plus sign in the dial string
				// by loop through
				// the network portion, post dial portion 1, post dial portion
				// 2... etc. if
				// applied
				do {
					...
					// Concatenates the string that is converted from network
					// portion
					if (!TextUtils.isEmpty(networkDialStr)) {
						...
						// This should never happen since we checked the if
						// dialStr is null
						// and if it contains the plus sign in the beginning of
						// this function.
						// The plus sign is part of the network portion.
						Log.e("checkAndProcessPlusCode: null newDialStr",
								networkDialStr);
						return dialStr;
					}
					...
					if (!TextUtils.isEmpty(postDialStr)) {
						...
						if (dialableIndex >= 1) {
							retStr = appendPwCharBackToOrigDialStr(
									dialableIndex, retStr, postDialStr);
							// Skips the P/W character, extracts the dialable
							// portion
							tempDialStr = postDialStr.substring(dialableIndex);
						} else {
							// Non-dialable character such as P/W should not be
							// at the end of
							// the dial string after P/W processing in
							// CdmaConnection.java
							// Set the postDialStr to "" to break out of the
							// loop
							if (dialableIndex < 0) {
								postDialStr = "";
							}
					...
					if (DBG)
						log("checkAndProcessPlusCode,postDialStr="
								+ postDialStr);
				} while (!TextUtils.isEmpty(postDialStr)
						&& !TextUtils.isEmpty(tempDialStr));
			} else {
				// TODO: Support NANP international conversion and other
				// telephone numbering plans.
				// Currently the phone is never used in non-NANP system, so
				// return the original
				// dial string.
	 ...
	 * rather than a regular PSTN phone number, based on whether or not the
	 * number contains an "@" character.
	 * 
	 * @hide
	 * @param number
	...
	/**
	 * @return the "username" part of the specified SIP address, i.e. the part
	 *         before the "@" character (or "%40").
	 * 
	 * @param number
	 *            SIP address of the form "username@domainname" (or the
	 ...
	 * If the number format is 1)+1NANP,remove +, 2)other than +1NANP, any +
	 * numbers,replace + with the current IDP
	 */
	private static String processPlusCodeWithinNanp(String networkDialStr) {
		String retStr = networkDialStr;

		if (DBG)
			log("processPlusCodeWithinNanp,networkDialStr=" + networkDialStr);
		// If there is a plus sign at the beginning of the dial string,
		// Convert the plus sign to the default IDP since it's an international
		// number
		...
		if (DBG)
			log("processPlusCodeWithinNanp,retStr=" + retStr);
		return retStr;
	}
