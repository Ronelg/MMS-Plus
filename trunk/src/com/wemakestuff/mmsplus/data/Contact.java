...
import com.wemakestuff.mmsplus.util.ContactInfoCache;
import com.wemakestuff.mmsplus.util.LogTag;
import com.wemakestuff.mmsplus.util.MessageUtils;
import com.wemakestuff.mmsplus.util.TaskStack;
...
public class Contact {
	...
	// private static final ContentObserver sContactsObserver = new
	...
	private static final ContentObserver sPresenceObserver = new ContentObserver(
			new Handler()) {
		@Override
		public void onChange(boolean selfUpdate) {
			if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
	...
	private String mNameAndNumber; // for display, e.g. Fred Flintstone
									// <670-782-1123>
	...
	public synchronized String toString() {
		return String
				.format("{ number=%s, name=%s, nameAndNumber=%s, label=%s, person_id=%d }",
	...
	public static Contact get(String number, boolean canBlock) {
		if (V)
			logWithTrace("get(%s, %s)", number, canBlock);

		if (TextUtils.isEmpty(number)) {
			throw new IllegalArgumentException(
					"Contact.get called with null or empty number");
		}
	...
	public static void invalidateCache() {
		...
		// force invalidate the contact info cache, so we will query for fresh
		// info again.
		// This is so we can get fresh presence info again on the screen, since
		// the presence
		// info changes pretty quickly, and we can't get change notifications
		// when presence is
		// updated in the ContactsProvider.
		ContactInfoCache.getInstance().invalidateCache();

		// While invalidating our local Cache doesn't remove the contacts, it
		// will mark them
		// stale so the next time we're asked for a particular contact, we'll
		// return that
		// stale contact and at the same time, fire off an asyncUpdateContact to
		// update
		// that contact's info in the background. UI elements using the contact
		// typically
		// call addListener() so they immediately get notified when the contact
		// has been
	...
	private static boolean contactChanged(Contact orig,
			ContactInfoCache.CacheEntry newEntry) {
		// The phone number should never change, so don't bother checking.
		// TODO: Maybe update it if it has gotten longer, i.e. 650-234-5678 ->
		// +16502345678?
		...
		if (!oldName.equals(newName)) {
			if (V)
				Log.d(TAG, String.format("name changed: %s -> %s", oldName,
						newName));
			return true;
		}
		...
		if (!oldLabel.equals(newLabel)) {
			if (V)
				Log.d(TAG, String.format("label changed: %s -> %s", oldLabel,
						newLabel));
			return true;
		}
	...
	/**
	 * Handles the special case where the local ("Me") number is being looked
	 * up. Updates the contact with the "me" name and returns true if it is the
	...
	private static boolean handleLocalNumber(Contact c) {
		if (MessageUtils.isLocalNumber(c.mNumber)) {
			c.mName = Cache.getContext().getString(
	...
	private static void asyncUpdateContact(final Contact c, boolean canBlock) {
		...
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			log("asyncUpdateContact for " + c.toString() + " canBlock: "
					+ canBlock + " isStale: " + c.mIsStale);
		}
	...
	public synchronized void dumpListeners() {
		int i = 0;
		Log.i(TAG, "[Contact] dumpListeners(" + mNumber + ") size="
				+ mListeners.size());
	...
	public static void init(final Context context) {
		...
		//TODO: MMS+ Change this to something that works, maybe?
		//RecipientIdCache.init(context);

		// it maybe too aggressive to listen for *any* contact changes, and
		// rebuild MMS contact
		// cache each time that occurs. Unless we can get targeted updates for
		// the contacts we
		// care about(which probably won't happen for a long time), we probably
		// should just
	...
	public static void startPresenceObserver() {
		Cache.getContext()
				.getContentResolver()
				.registerContentObserver(Presence.CONTENT_URI, true,
						sPresenceObserver);
	}

	public static void stopPresenceObserver() {
		Cache.getContext().getContentResolver()
				.unregisterContentObserver(sPresenceObserver);
	}
	...
	private static class Cache {
		...
		static Contact get(String number) {
			...
			synchronized (sInstance) {
				for (Contact c : sInstance.mCache) {
					...
					// if the numbers are an exact match (i.e. Google SMS), or
					// if the phone
					// number comparison returns a match, return the contact.
					if (number.equals(c.mNumber)
							|| PhoneNumberUtils.compare(number, c.mNumber)) {
						return c;
					}
				}
				return null;
		...
		static void invalidate() {
			// Don't remove the contacts. Just mark them stale so we'll update
			// their
