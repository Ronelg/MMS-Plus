...
public class RecipientIdCache {
	...
	private static Uri sAllCanonical = Uri
		...
		for (Contact contact : contacts) {
			...
			synchronized (sInstance) {
				...
				if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
					Log.d(TAG, "[RecipientIdCache] updateNumbers: contact="
							+ contact + ", wasModified=true, recipientId="
							+ recipientId);
	...
	private void updateCanonicalAddressInDb(long id, String number) {
		if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			Log.d(TAG, "[RecipientIdCache] updateCanonicalAddressInDb: id="
					+ id + ", number=" + number);
		}
		...
		final Uri uri = ContentUris.withAppendedId(sSingleCanonicalAddressUri,
				id);
	 ...
	 */
	public static String getSingleAddressFromCanonicalAddressInDb(
			final Context context, final String recipientId) {
		Cursor c = SqliteWrapper.query(
				context,
				context.getContentResolver(),
				ContentUris.withAppendedId(sSingleCanonicalAddressUri,
						Long.parseLong(recipientId)), null, null, null, null);
		if (c == null) {
	...
	// used for unit tests
	public static void insertCanonicalAddressInDb(final Context context,
			String number) {
		if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			Log.d(TAG, "[RecipientIdCache] insertCanonicalAddressInDb: number="
					+ number);
		}
